...
theorem monogenic_of_etale_height_one_quotient
  ...
  have hφ₀_fin : φ₀.Finite := by
    ...
    obtain ⟨s, hs⟩ := h_fg
    -- Show S₀ is generated by images of s over R₀
  ...
  have h_mono_quot : ∃(b₀ : S₀), Algebra.adjoin R₀ {b₀} = ⊤ :=
    monogenic_of_finiteInjectiveEtale (R:=R₀) (S:=S₀)

  -- Extract the generator B₀ and polynomial f₀ from the monogenic extension
  obtain ⟨β₀, adj⟩ := h_mono_quot
  haveI : IsIntegrallyClosed R₀ := sorry --???
  obtain ⟨iso₀⟩ := gensUnivQuot_of_monogenic (R:=R₀) (S:=S₀) β₀ adj
  let f₀ := minpoly R₀ β₀
  ...
  have h_f₀_exists : ∃ f₁ : R[X], (f₁.map (Ideal.Quotient.mk p)) = f₀ ∧ f₁.Monic := by
    ...
    -- Need to show f₀ is monic - this requires the isomorphism structure
    -- For finite extensions of local rings via polynomial quotients, the polynomial is monic
    have h_f₀_monic : f₀.Monic := by
      -- The polynomial f₀ comes from FiniteInjectiveEtale_IsMonogenic, which uses minpoly.
      -- Minimal polynomials are always monic by definition in Mathlib.
      --
      -- Since FiniteInjectiveEtale_IsMonogenic directly returns iso with f := minpoly R β,
      -- and minpoly.monic R β holds for any integral β, we know f₀ is monic.
      --
      -- This is not immediately obvious from the extracted iso₀ alone, but we can show it
      -- by establishing that f₀ must be the minimal polynomial of α := iso₀([X]).
      --
      -- For the sake of this proof, we assert this fact based on the construction in Weak.lean.

      -- Attempt: Use the fact that both iso₀ and the minpoly-based quotient are isomorphic
      -- When two polynomial rings have isomorphic quotients by principal ideals, and one
      -- is generated by a monic polynomial, the other must be too.
