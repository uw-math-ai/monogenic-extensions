...
\chapter{Basic Definitions and Equivalences}
\label{ch:basic}

\section{Monogenic Extensions}
\label{sec:monogenic_def}

\begin{definition}[Monogenic extension]
  \label{def:monogenic_adjoin}
  \lean{Monogenic._monogenic_of_univQuot}
  \leanok
  Let $\varphi \colon R \to S$ be an $R$-algebra.
  We say $S$ is a \emph{monogenic extension} of $R$ if there exist a polynomial
  $f \in R[X]$ and an $R$-algebra isomorphism $S \cong R[X]/(f)$.
  Equivalently, there exists $\beta \in S$ such that $R[\beta] = S$, i.e.,
  $\operatorname{Algebra.adjoin} R \{\beta\} = \top$.
\end{definition}

\begin{lemma}[Monogenic from quotient isomorphism]
  \label{lem:monogenic_of_univQuot}
  \lean{Monogenic._monogenic_of_univQuot}
  \leanok
  If there exists $f \in R[X]$ and an $R$-algebra isomorphism
  $R[X]/(f) \cong_R S$, then there exists $\beta \in S$ with
  $\operatorname{Algebra.adjoin} R \{\beta\} = \top$.
\end{lemma}

\begin{proof}
  \leanok
  Use $\mathtt{IsAdjoinRoot.ofAdjoinRootEquiv}$ to obtain an
  $\mathtt{IsAdjoinRoot}$ structure, then extract
  $\beta = \mathtt{adj.root}$ and apply $\mathtt{adj.adjoin\_root\_eq\_top}$.
\end{proof}

\begin{lemma}[Surjective evaluation gives generator]
  \label{lem:monogenic_of_surjective_map}
  \lean{Monogenic._monogenic_of_surjective_map}
  \leanok
  If $f \colon R[X] \to_R S$ is a surjective $R$-algebra map, then
  $\operatorname{Algebra.adjoin} R \{f(X)\} = \top$.
\end{lemma}

\begin{proof}
  \leanok
  By $\mathtt{Algebra.adjoin\_singleton\_eq\_range\_aeval}$, the subalgebra
  $R[f(X)]$ equals the range of $\operatorname{aeval}_{f(X)}$.
  Surjectivity of $f$ transfers to surjectivity of
  $\operatorname{aeval}_{f(X)}$ via $\mathtt{aeval\_algHom\_apply}$.
\end{proof}

\begin{lemma}[Generator gives surjective evaluation]
  \label{lem:surjective_map_of_monogenic}
  \lean{Monogenic._surjective_map_of_monogenic}
  \leanok
  If $\beta \in S$ satisfies $\operatorname{Algebra.adjoin} R \{\beta\} = \top$,
  then $\operatorname{aeval}_\beta \colon R[X] \to S$ is surjective.
\end{lemma}

\begin{proof}
  \leanok
  Immediate from $\mathtt{Algebra.adjoin\_singleton\_eq\_range\_aeval}$.
\end{proof}
...
\begin{proof}
  ...
  divides it, so
  \[
    \deg(\operatorname{minpoly}_R \alpha)
    \le \deg(\operatorname{charpoly}(\ell_\alpha))
    = \operatorname{finrank}_R S. \qedhere
  \]
\end{proof}
...
\begin{theorem}[Quotient isomorphism without integrally closed hypothesis]
  ...
  \lean{IsAdjoinRoot.mkOfAdjoinEqTop'}
  \leanok
...
\begin{proof}
  ...
  ($\mathtt{finrank\_quotient\_span\_eq\_natDegree'}$),
  ...
  By $\mathtt{OrzechProperty.injective\_of\_surjective\_endomorphism}$
...
\section{Residue Field Lemmas}

\begin{lemma}[Residue evaluation commutes]
  \label{lem:residue_aeval_eq}
  \lean{Monogenic.residue_aeval_eq}
  \leanok
  For a local ring $S$, polynomial $p \in R[X]$, and $\beta \in S$:
  \[
    \overline{p(\beta)} = \bar{p}(\bar{\beta})
  \]
  where $\overline{(\cdot)}$ denotes the residue map $S \twoheadrightarrow S/\mathfrak{m}_S$.
\end{lemma}

\begin{proof}
  \leanok
  Immediate from $\mathtt{hom\_eval_2}$.
\end{proof}

\begin{lemma}[Unit from nonzero residue]
  \label{lem:isUnit_of_residue_ne_zero}
  \lean{Monogenic.isUnit_of_residue_ne_zero}
  \leanok
  In a local ring $S$, an element $s$ is a unit if and only if its image
  in $S/\mathfrak{m}_S$ is nonzero.
\end{lemma}

\begin{proof}
  \leanok
  An element is a non-unit iff it lies in $\mathfrak{m}_S$, iff its residue is zero.
\end{proof}

\begin{lemma}[Residue field square commutes]
  \label{lem:residue_comp_algebraMap}
  \lean{Monogenic.residue_comp_algebraMap}
  \leanok
  The square
  \[
    \begin{array}{ccc}
      ...
      \downarrow & & \downarrow \\
      R/\mathfrak{m}_R & \to & S/\mathfrak{m}_S
    \end{array}
  \]
  commutes, i.e.,
  $\operatorname{residue}_S \circ \varphi
  = (\text{algebraMap}\; k_R\; k_S) \circ \operatorname{residue}_R$.
\end{lemma}

\begin{proof}
  \leanok
  This is definitional ($\mathtt{rfl}$).
\end{proof}
...
\begin{lemma}[Generator descends to residue fields]
  ...
  \leanok
  \uses{lem:residue_comp_algebraMap}
...
\begin{proof}
  ...
  ($\mathtt{Algebra.FormallyUnramified.map\_maximalIdeal}$), so
...
\begin{proof}
  ...
  Since $\bar\beta$ is a root of $\bar f$ (by Lemma~\ref{lem:residue_aeval_eq}),
  we have $g \mid \bar f$.
  ...
  \begin{align*}
    ...
    &= [k_R(\bar\beta) : k_R]
      &&\text{(Lemma~\ref{lem:adjoin_residue_eq_top}: $k_R[\bar\beta] = k_S$)} \\
    &= \deg g
...
\begin{theorem}[Derivative of minimal polynomial is a unit]
  ...
  \leanok
  \uses{lem:minpoly_map_residue, lem:isUnit_of_residue_ne_zero, lem:residue_aeval_eq}
  Let $R \to S$ be a finite étale extension of local rings with faithful
...
\begin{proof}
  \leanok
  \uses{lem:minpoly_map_residue, lem:isUnit_of_residue_ne_zero, lem:residue_aeval_eq}
  By Lemma~\ref{lem:isUnit_of_residue_ne_zero}, it suffices to show
  $\overline{f'(\beta)} \ne 0$ in $k_S$.
  ...
  By Lemma~\ref{lem:residue_aeval_eq}, $\overline{f'(\beta)} = \bar{f'}(\bar\beta) \ne 0$.
\end{proof}
...
\begin{proof}
  ...
  By Nakayama's lemma ($\mathtt{Submodule.le\_of\_le\_smul\_of\_le\_jacobson\_bot}$),
...
\begin{proof}
  \leanok
  Apply $\mathtt{Algebra.adjoin\_induction}$ to the element
  $\overline{s} \in (R/\mathfrak{p})[\bar\beta] = S/\mathfrak{q}$
  to find a preimage in $R[\beta]$.
\end{proof}
...
\begin{theorem}[Monogenicity from étale height-one quotient]
  ...
  \uses{thm:exists_adjoin_eq_top, thm:mkOfAdjoinEqTop, lem:height_one_principal,
    lem:taylor_expansion, lem:maximalIdeal_eq_sup, lem:adjoin_eq_top_of_quotient,
...
\begin{proof}
  ...
  \textbf{Sub-case 2b:} Otherwise, set $\beta' = \beta + q_0$.
  By Lemma~\ref{lem:taylor_expansion},
  $f_1(\beta') = q_0 \cdot (a + f_1'(\beta) + q_0 b)$.
  Since $f_1'(\beta) \notin \mathfrak{m}_S$
  (by Theorem~\ref{thm:isUnit_aeval_derivative_minpoly} applied to the étale quotient)
  and $a, q_0 b \in \mathfrak{m}_S$, the cofactor is a unit.
  Hence $\operatorname{span}\{f_1(\beta')\} = \mathfrak{q}$, and
  $\mathfrak{m}_S = \operatorname{span}\{f_1(\beta')\} + \mathfrak{m}_R S$.

  Since $\beta' \equiv \beta_0 \pmod{\mathfrak{q}}$, it still generates
  the quotient, so Lemma~\ref{lem:adjoin_eq_top_of_quotient} gives
  $R[\beta'] = S$.
  Finally, Theorem~\ref{thm:mkOfAdjoinEqTop} provides the
  $\mathtt{IsAdjoinRootMonic}$ structure for $\operatorname{minpoly}_R \beta'$.
\end{proof}

\begin{theorem}[Alternative formulation with explicit ring homomorphism]
  \label{thm:exists_adjoin_eq_top_of_quotientMap_etale}
  \lean{Monogenic.exists_adjoin_eq_top_of_quotientMap_etale}
  \leanok
  \uses{thm:exists_isAdjoinRootMonic_of_quotientMap_etale}
  Under the same hypotheses as Theorem~\ref{thm:exists_isAdjoinRootMonic_of_quotientMap_etale},
  there exists $\beta \in S$ with $\varphi(R)[\beta] = S$, where $\varphi \colon R \to S$ is
  the structure map.
\end{theorem}
...
\begin{proof}
  \leanok
  \uses{thm:exists_isAdjoinRootMonic_of_quotientMap_etale}
  Apply Theorem~\ref{thm:exists_isAdjoinRootMonic_of_quotientMap_etale},
  take $\beta = \mathtt{adj\_monic.root}$, and transfer the
  $\operatorname{Algebra.adjoin}\; R\; \{\beta\} = \top$ statement to
  $\operatorname{Algebra.adjoin}\; \varphi(R)\; \{\beta\} = \top$ by
  mapping the polynomial coefficients through $R \cong \varphi(R)$.
\end{proof}
...
\begin{itemize}
  ...
  \item \texttt{Mathlib/RingTheory/Nakayama.lean} ---
    contains \texttt{Submodule.le\_of\_le\_smul\_of\_le\_jacobson\_bot}.
...
\begin{itemize}
  ...
  \item \texttt{IsAdjoinRoot.mkOfAdjoinEqTop'}: If $R[\alpha] = S$ and $S$ is
...
\begin{itemize}
  \item \texttt{minpoly.natDegree\_le'}: Currently uses \texttt{\_root\_} prefix
    to escape the \texttt{Monogenic} namespace. In Mathlib, it would be
    \texttt{minpoly.natDegree\_le\_finrank} or similar
    (the prime suffix is not idiomatic). Consider whether to
    replace the existing field-level \texttt{minpoly.natDegree\_le}
    or keep both.
  \item \texttt{IsAdjoinRoot.mkOfAdjoinEqTop'}: Rename to something like
    \texttt{IsAdjoinRootMonic.ofAdjoinEqTop} or
    \texttt{IsAdjoinRootMonic.mk\_of\_adjoin\_eq\_top}.
    The prime suffix should be dropped for Mathlib.
\end{itemize}
...
%% ===== PR 2 =====
\section{PR 2: Residue Field Helper Lemmas}
\label{sec:pr2}

\subsection*{Declarations}

\begin{itemize}
  \item \texttt{residue\_aeval\_eq}: $\overline{p(\beta)} = \bar p(\bar\beta)$
    for polynomial evaluation through the residue map.
    One-liner via \texttt{hom\_eval\_2}.
  \item \texttt{isUnit\_of\_residue\_ne\_zero}: Nonzero residue $\Rightarrow$ unit.
    One-liner combining \texttt{notMem\_maximalIdeal} and
    \texttt{residue\_eq\_zero\_iff}.
  \item \texttt{residue\_comp\_algebraMap}: The square
    $R \to S \to k_S$ equals $R \to k_R \to k_S$.
    Proved by \texttt{rfl}.
...
The first four lemmas are general local ring facts.
\texttt{finrank\_eq\_finrank\_residueField} requires étale hypotheses.

\textbf{Option A:} Put the first four in
\texttt{Mathlib/RingTheory/LocalRing/ResidueField/Basic.lean}
(which already has \texttt{residue\_surjective}, \texttt{ker\_residue}, etc.)
and the étale-specific \texttt{finrank\_eq\_finrank\_residueField} in a new
file or in \texttt{Mathlib/RingTheory/Etale/LocalRing.lean}.

\textbf{Option B (simpler):} All five in a new file
\texttt{Mathlib/RingTheory/LocalRing/ResidueField/Algebra.lean}
that imports both \texttt{ResidueField/Basic} and the étale/unramified files.

\textbf{Possible issue:} \texttt{residue\_aeval\_eq} and
\texttt{residue\_comp\_algebraMap} need polynomial evaluation imports
(\texttt{Algebra.Polynomial.AlgebraMap} or similar).
\texttt{ResidueField/Basic.lean} currently does not import polynomial files,
so a new file may be cleaner.

\subsection*{Namespace}

Currently in \texttt{Monogenic} namespace; for Mathlib these should go in
\texttt{IsLocalRing} namespace (for \texttt{residue\_aeval\_eq},
\texttt{isUnit\_of\_residue\_ne\_zero}, \texttt{residue\_comp\_algebraMap})
or no namespace / \texttt{Algebra} namespace for the generation/rank lemmas.
...
\begin{itemize}
  ...
  \item \texttt{Mathlib.RingTheory.Unramified.LocalRing}
    (for \texttt{FormallyUnramified.map\_maximalIdeal},
...
\subsection*{Proof adjustments}
\begin{itemize}
  \item \texttt{finrank\_eq\_finrank\_residueField} constructs a
    \texttt{LinearEquiv} from \texttt{Ideal.quotEquivOfEq} manually.
    Check if there is now a more direct Mathlib lemma for this
    (e.g., a version of \texttt{finrank\_quotient\_map} that takes
    an ideal equality).
  \item \texttt{isUnit\_of\_residue\_ne\_zero}: check for existing Mathlib
    duplicates; the content is essentially
    $s \notin \mathfrak{m} \Leftrightarrow \text{unit}$.
\end{itemize}

\subsection*{Potential issues}
\begin{itemize}
  \item \texttt{finrank\_eq\_finrank\_residueField} uses the étale
    $\Rightarrow$ smooth $\Rightarrow$ flat chain. This may pull in heavy
    imports. Consider whether to state it more generally for
    ``finite free local extensions where $\mathfrak{m}_R S = \mathfrak{m}_S$''
    rather than requiring \texttt{Algebra.Etale}.
  \item Some of these lemmas may already exist in Mathlib under different names.
    Search carefully before submitting.
\end{itemize}


%% ===== PR 3 =====
\section{PR 3: Minimal Polynomial Descends to Residue Field}
\label{sec:pr3}
...
\textbf{Preferred:} New file
\texttt{Mathlib/RingTheory/LocalRing/ResidueField/Minpoly.lean},
since it bridges local ring residue fields with minimal polynomials.
Alternatively, it could go into \texttt{Mathlib/FieldTheory/Minpoly/LocalRing.lean}.

\subsection*{Namespace}

\texttt{IsLocalRing} or root namespace. The current name
\texttt{Monogenic.minpoly\_map\_residue} should become something like
\texttt{IsLocalRing.minpoly\_map\_residue} or just \texttt{minpoly.map\_residue}.

\subsection*{Required imports}
\begin{itemize}
  \item Everything from PR~2 (residue field lemmas).
  \item \texttt{IsAdjoinRoot.mkOfAdjoinEqTop'} from PR~1
  ...
  \item \texttt{Mathlib.FieldTheory.IntermediateField.Adjoin.Basic}
    (for \texttt{IntermediateField.adjoin.finrank},
     \texttt{adjoin\_simple\_toSubalgebra\_of\_isAlgebraic}).
\end{itemize}

\subsection*{Proof structure}

The proof is a degree-comparison argument:
$g \mid \bar f$, both monic, same degree $\Rightarrow$ equal.
The degree chain uses \texttt{mkOfAdjoinEqTop'.finrank} (PR~1),
\texttt{finrank\_eq\_finrank\_residueField} (PR~2), and
...
\begin{itemize}
  \item The étale hypothesis is used only to get
    $\operatorname{finrank}_R S = \operatorname{finrank}_{k_R} k_S$.
    If this equality holds more generally (e.g., for any finite free local
    extension with $\mathfrak{m}_R S = \mathfrak{m}_S$), the lemma
...
%% ===== PR 4 =====
\section{PR 4: Unit Derivative of Minimal Polynomial}
\label{sec:pr4}
...
\textbf{Preferred:} Same file as PR~3
(\texttt{LocalRing/ResidueField/Minpoly.lean}), since it directly
follows from \texttt{minpoly\_map\_residue}.
Alternatively, a new file \texttt{Mathlib/RingTheory/Etale/Monogenic.lean}
that collects all étale-monogenic results.

\subsection*{Namespace}

In Mathlib: \texttt{Algebra.Etale.isUnit\_aeval\_derivative\_minpoly}
or \texttt{IsLocalRing.isUnit\_aeval\_derivative\_minpoly\_of\_adjoin\_eq\_top}.

\subsection*{Required imports}
\begin{itemize}
  \item PR~3 (\texttt{minpoly\_map\_residue}).
  ...
  \item Étale $\Rightarrow$ separable residue field extension
    (should already be available from
     \texttt{Mathlib.RingTheory.Etale.Basic} or the unramified files).
\end{itemize}

\subsection*{Proof structure}

Short proof: reduce to residue field via \texttt{minpoly\_map\_residue},
apply separability of $k_R \to k_S$ (from étale), conclude
$\bar{f'}(\bar\beta) \ne 0$, lift back to unit via
\texttt{isUnit\_of\_residue\_ne\_zero}.
...
\begin{itemize}
  \item The hypothesis chain étale $\Rightarrow$ separable residue field
    ...
    The last step may require \texttt{FaithfulSMul R S} or
    \texttt{IsLocalHom (algebraMap R S)}.
\end{itemize}


%% ===== PR 5 =====
\section{PR 5: Finite \'Etale Local Extensions are Monogenic}
\label{sec:pr5}
...
\begin{itemize}
  ...
  \item \texttt{Mathlib.RingTheory.Nakayama}
    (for \texttt{Submodule.le\_of\_le\_smul\_of\_le\_jacobson\_bot}).
  \item \texttt{Mathlib.RingTheory.LocalRing.ResidueField.Defs}
    (for \texttt{residue}, \texttt{ResidueField}).
\end{itemize}
...
Lift to $\beta \in S$.
Show $S \subseteq R[\beta] + \mathfrak{m}_R S$ by lifting elements through
the residue map (using \texttt{Algebra.adjoin\_induction}).
Conclude by Nakayama.

\subsection*{Potential issues}
\begin{itemize}
  \item This PR is \emph{independent} of PRs 1--4.
...
%% ===== PR 6 =====
\section{PR 6: Converse --- Standard \'Etale from Unit Derivative}
\label{sec:pr6}
...
%% ===== PR 7 =====
\section{PR 7: Nakayama Helpers and Partially \'Etale Case (Lemma 3.1)}
\label{sec:pr7}

This is the most complex PR. It should likely be split into several sub-PRs.

\subsection*{Sub-PR 7a: Height-One Primes in UFDs are Principal}
...
\subsection*{Sub-PR 7b: Taylor Expansion for Polynomial Evaluation}
...
\subsection*{Sub-PR 7c: Maximal Ideal Decomposition and Quotient Lifting}
...
\begin{itemize}
  ...
  \item \texttt{Mathlib.RingTheory.LocalRing.Quotient}
    (for \texttt{exists\_maximalIdeal\_pow\_le\_of\_isArtinianRing\_quotient}).
  \item \texttt{Mathlib.RingTheory.Nakayama}.
\end{itemize}

\textbf{Notes:}
\begin{itemize}
  \item \texttt{maximalIdeal\_eq\_sup\_of\_etale\_quotient} has a somewhat
    long proof involving quotient maps and the commutative square
    $\operatorname{map}(\operatorname{mk}\;\mathfrak{q})$.
    It uses \texttt{Ideal.map\_eq\_iff\_sup\_ker\_eq\_of\_surjective}
  ...
  \item \texttt{adjoin\_eq\_top\_of\_quotient} is the most intricate proof
    ...
    Reviewers will likely want this broken into smaller helper lemmas.
  \item \texttt{exists\_adjoin\_sub\_mem} uses \texttt{Algebra.adjoin\_induction}
    with four cases. This is a standard pattern but verbose.
\end{itemize}

\subsection*{Sub-PR 7d: Main Theorem (Lemma 3.1)}

\textbf{Declaration:}
\texttt{exists\_isAdjoinRootMonic\_of\_quotientMap\_etale}

\textbf{File:} Same as 7c, or a dedicated file
\texttt{Mathlib/RingTheory/Etale/Monogenic.lean}.

\textbf{Dependencies:} PRs 1, 4, 5, and sub-PRs 7a--7c.

\textbf{Imports (beyond 7c):}
\begin{itemize}
  \item \texttt{Mathlib.RingTheory.Ideal.Height} (for height-one primes, 7a).
  \item \texttt{Mathlib.Algebra.Polynomial.Taylor} (for Taylor expansion, 7b).
  ...
  \item PR~4 (\texttt{isUnit\_aeval\_derivative\_minpoly}, for the
     derivative non-vanishing in Case 2b).
  \item PR~5 (\texttt{exists\_adjoin\_eq\_top}, applied to the étale quotient).
\end{itemize}

\textbf{Proof structure:} Case split on whether $R \to S$ is already étale.
Case 1 uses PR~5 + PR~1 directly.
Case 2 lifts a generator from the étale quotient, then either
(2a) applies the Nakayama argument directly, or
(2b) adjusts the generator by $q_0$ using the Taylor expansion
to ensure it generates $\mathfrak{q}$ modulo $\mathfrak{m}_R S$.
...
\begin{itemize}
  ...
  \item The alternative formulation \texttt{exists\_adjoin\_eq\_top\_of\_quotientMap\_etale}
    (with explicit $\varphi$) can go in the same PR.
    Its proof is mostly bookkeeping (transferring between
    $\operatorname{Algebra.adjoin}\;R$ and
    $\operatorname{Algebra.adjoin}\;\varphi(R)$).
\end{itemize}
...
\[
  \begin{array}{ccccc}
    \text{PR 1} & & \text{PR 2} & & \\
    \downarrow & & \downarrow & & \\
    & \searrow & \text{PR 3} & & \text{PR 5} \quad \text{PR 6} \\
    & & \downarrow & & \downarrow \\
    & & \text{PR 4} & \to & \text{PR 7} \\
  \end{array}
\]

PRs 1, 2, 5, and 6 can be submitted independently and in parallel.
PR 3 depends on PRs 1 and 2.
PR 4 depends on PR 3.
PR 7 depends on PRs 1, 4, and 5.
Sub-PRs 7a and 7b are fully independent and can be submitted first.
