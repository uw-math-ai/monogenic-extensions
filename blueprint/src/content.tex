...
\begin{proof}
  ...
  $\overline{f'(\beta)} \ne 0$ in $k_S$
  ($\mathtt{IsLocalRing.residue\_ne\_zero\_iff\_isUnit}$).
...
\begin{lemma}[Adjustment by principal generator]
  ...
  \leanok
  \uses{lem:taylor_expansion, lem:adjoin_eq_top_of_quotient, thm:mkOfAdjoinEqTop}
  ...
  and let $\beta \in S$, $f_1 \in R[X]$ satisfy:
  \begin{enumerate}
    \item $f_1(\beta) \in \mathfrak{q}$,
    \item $f_1'(\beta) \notin \mathfrak{m}_S$,
    \item $\mathfrak{m}_S = \mathfrak{q} + \mathfrak{m}_R S$,
    \item $\bar\beta$ generates $S/\mathfrak{q}$ over $R/(\mathfrak{q} \cap R)$,
    \item $f_1(\beta)$ does \emph{not} already generate $\mathfrak{m}_S$ modulo $\mathfrak{m}_R S$.
  \end{enumerate}
...
\begin{proof}
  ...
  Since $\beta' \equiv \bar\beta \pmod{\mathfrak{q}}$, it still generates
...
\begin{proof}
  ...
  \textbf{Sub-case 2b:} Otherwise, apply
  Lemma~\ref{lem:exists_isAdjoinRootMonic_of_principal_adjust}
  with $q_0$, $\beta$, $f_1$.
...
(2b) delegates to \texttt{exists\_isAdjoinRootMonic\_of\_principal\_adjust},
which adjusts the generator by $q_0$ using the Taylor expansion
